...
class DownloaderURLList(Downloader):
    ...
    def download_file(self, url, output_path, timeout=60):
        """Download a file from the given URL, save it, then unzip and delete the zip.
    ...
    def url_list_download(self, urls, base_dir, log_file="oleander_download.log", strip_prefix="thredds/fileServer/oceansites/", num_threads=4):
        ...
        Args:
            ...
            log_file (str, optional): Path to the log file. Defaults to 'oleander_download.log'.
            strip_prefix (str, optional): Prefix to strip from URL paths. Defaults to 'thredds/fileServer/oceansites/'.
            num_threads (int, optional): Number of threads for downloading. Defaults to 4.

        Returns:
            bool: True if download process completes successfully, False otherwise.
        """
        # Configure logging
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            future_to_url = {
                executor.submit(self.download_file, url, self.create_directory_structure(base_dir, url, strip_prefix)): url
                for url in urls
            }
